...
function addResultsBS(config, results) {
    ...
    newDiv.addEventListener('click', e => {
        ...
        if ('function' === typeof window.resultHandlerBS) {
            resultHandlerBS(
                config.name,
                results[selectedValue.dataset.resultid]
            )
        }
        clearListBS()
    })
...
function handleInputBS(e, config) {
    ...
    const fetchURL = config.fetchURL.replace(
        '{term}',
    ...
    fetch(fetchURL)
        ...
        .then(response => {
            ...
            if (!Array.isArray(results)) {
                console.log(
                    'Was expecting an array from the Fetch API - Setting to Empty'
                )
                results = []
            }
...
function handleKeyDownBS(e, config) {
    ...
    if (autocompleteBSDiv.dataset.forinputbs == e.target.id) {
        ...
        const keyPressed = parseInt(e.keyCode)
        let keyAction = ''

        if (keyPressed === 40 || keyPressed === 9) keyAction = 'down'
        if (keyPressed === 38 || (e.shiftKey && keyPressed == 9))
            keyAction = 'up'
        ...
        switch (keyAction) {
            ...
            case 'enter':
                ...
                config.targetID.value =
                    results[currentPosition - 1][config.fetchMap.id]
                clearListBS()
                if ('function' === typeof window.resultHandlerBS) {
...
function clickCheckBS(e, config) {
    ...
    if (sourceBS == e.target.id) {
        ...
        console.log(
            'Clicked outside target with an active list - Send back to input'
        )
